...
//var soup = new JSSoup(document.body);  // Parsing current page (class details)
getHtmlString('https://apps.lbcc.edu/schedule/scheduleDetail.cfm?term=1625&courseID=009185&semester=Fall&Descr=Intro%20to%20Computer%20Architecture&strAction=&campus=&subject=CS&catalog_nbr=51').then(data =>
{
    var soup = new JSSoup(data.toString())
    allLinks = soup.findAll('a'); // All <a> tags from body
...
//     {
//         profileLinks[i] = profileLinks[i].replace('phonebook', 'apps').replace('phonedir', 'phonebook');
//     }
    ...
    for (i = 0; i < profNames.length; i++)
    {
        ...
        // getHtmlString(profNames[i]).then(data =>
        // {
        //     var profileSoup = new JSSoup(data.toString());
        ...
        //     var cell = profileSoup.find('td', {id:'emp0', class:'expand-me bold-text'});
            ...
            getHtmlString(searchpageURL).then(data => 
            {
                ...
                getHtmlString(ratingpageURL).then(data =>
                {
                    ...
                    console.log(list[0] + profNames[i]);
